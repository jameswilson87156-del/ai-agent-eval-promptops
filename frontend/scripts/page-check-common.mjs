...
export async function checkShowcasePage({
  slug,
  view,
  frontendPort,
  shellSelector,
  ...
  const localOutputPath = path.join(repoRoot, '.local', `promptops-${slug}-16x9-check.png`)
  const formalOutputPath = path.join(repoRoot, 'docs', 'images', `${slug}.png`)
  const children = []
  let browser
  ...
  try {
    ...
    assert.ok(response?.ok(), `${slug} did not return a successful document response`)
    await page.locator('[data-ready="true"]').waitFor({ state: 'visible', timeout: 30_000 })
    await page.locator(shellSelector).waitFor({ state: 'visible', timeout: 30_000 })
    ...
    assert.ok(
      ...
      `${slug} overflows horizontally: ${JSON.stringify(dimensions)}`,
    )
    assert.ok(
      Math.max(dimensions.documentHeight, dimensions.bodyHeight) <= viewport.height + 1,
      `${slug} requires vertical scrolling: ${JSON.stringify(dimensions)}`,
    )
    ...
    for (const { selector, expected, label } of countChecks) {
      assert.equal(await page.locator(selector).count(), expected, label ?? `${selector} count mismatch`)
    }

    await extraChecks?.(page)
    assert.deepEqual(browserErrors, [], `${slug} emitted browser errors`)
    ...
    assert.ok(localMetadata.size > 10_000, `${slug} screenshot is unexpectedly small`)
    assert.equal(formalMetadata.size, localMetadata.size, 'Formal screenshot differs from checked screenshot')

    await context.close()
    process.stdout.write(`Captured ${viewport.width}x${viewport.height} ${slug} screenshot at ${localOutputPath}\n`)
    process.stdout.write(`Wrote verified project screenshot to ${formalOutputPath}\n`)
  } finally {
    await browser?.close()
    children.reverse().forEach(stopProcess)
    await waitForUnavailable(`http://127.0.0.1:${frontendPort}`)
  }
}
