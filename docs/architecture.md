...
`Eval Run` 保存一次版本与评测集组合的聚合结果；`Eval Case Result` 保存逐 Case 状态、评分、延迟、输出与解释；`Generation Trace` 保存输入/输出摘要和失败原因，支持按 Run 与 Case 查询。
...
真实 LLM Provider、Judge、任务队列、权限、审计和生产监控均未实现。后端仅返回是否需要 Review，人工 Review 结论仍只保存在当前页面，不能视为审批系统。
